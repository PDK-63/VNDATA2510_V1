...
{
    cfg_output(APP_STATUS_LED_GPIO, 0);
    cfg_output(APP_RELAY_GPIO, 0);
...
{
    s_led = on;
    if (APP_STATUS_LED_GPIO != GPIO_NUM_NC) gpio_set_level(APP_STATUS_LED_GPIO, on ? 1 : 0);
}
...
{
    s_relay = on;
    if (APP_RELAY_GPIO != GPIO_NUM_NC) gpio_set_level(APP_RELAY_GPIO, on ? 1 : 0);
}
