...
// static uint32_t s_alarm_call_req_seq = 0;
// static uint32_t s_alarm_call_inflight_seq = 0;
...
//     /* cac lenh config tu web setting */
...
static void update_do1_output_state(void)
{
    ...
    bool do1_should_on = s_alarm_active || power_lost;

    board_tca_write_pin(APP_DO1_TCA_PIN, do1_should_on);

    if (s_display_ready) {
        tm1638_set_led(&s_tm1638, 3, do1_should_on);
    }

    ESP_LOGI(TAG, "DO1 update: alarm=%d power_lost=%d => do1=%d",
             s_alarm_active ? 1 : 0,
             power_lost ? 1 : 0,
             do1_should_on ? 1 : 0);
}
...
static bool is_hum_low_trigger_now_internal(void)
{
    return s_humidity_valid && (s_last_humidity < s_runtime_cfg.hum_low_limit_pct);
}

static bool is_hum_high_trigger_now_internal(void)
{
    return s_humidity_valid && (s_last_humidity > s_runtime_cfg.hum_high_limit_pct);
}
...
static bool is_hum_low_restored_with_hyst(void)
{
    return s_humidity_valid && (s_last_humidity >= (s_runtime_cfg.hum_low_limit_pct + APP_HUM_HYST_PCT));
}

static bool is_hum_high_restored_with_hyst(void)
{
    return s_humidity_valid && (s_last_humidity <= (s_runtime_cfg.hum_high_limit_pct - APP_HUM_HYST_PCT));
}
...
static void trigger_alarm_immediately_after_limit_update_internal(bool temp_limit_changed, bool hum_limit_changed)
{
    ...
    if (temp_limit_changed && temp_alarm_now && !s_alarm_temp_was_active) {
    ...
    if (hum_limit_changed && hum_alarm_now && !s_alarm_hum_was_active) {
    ...
    /* DANG CO ALARM ROI NHUNG XUAT HIEN THEM NGUYEN NHAN MOI
       -> VAN PHAI GUI CANH BAO MOI NGAY */
...
//     s_alarm_last_notify_ms = esp_timer_get_time() / 1000;
//     s_alarm_notify_count = 1;

//     ESP_LOGW(TAG,
//              "new alarm cause while alarm active -> queue SMS+CALL temp_new=%d hum_new=%d temp=%.2f hum=%.2f",
//              new_temp_alarm ? 1 : 0,
//              new_hum_alarm ? 1 : 0,
//              s_last_temp1_c,
//              s_last_humidity);
...
static void build_power_lost_sms(char *out, size_t out_len)
{
    ...
    float hum_now  = s_humidity_valid ? s_last_humidity : 0.0f;
...
static void process_alarm_restored_sms(void)
{
    if (!s_alarm_restored_sms_pending || s_alarm_restored_sms_sent) {
        return;
    }
    ...
    if (!s_alarm_restored_sms_job_queued) {
        if (enqueue_modem_job(MODEM_JOB_SMS_ALARM_RESTORED)) {
            s_alarm_restored_sms_job_queued = true;
            s_last_alarm_restored_sms_try_ms = now_ms;
            ESP_LOGW(TAG, "queued alarm restored SMS");
        }
    }
}
...
//         ESP_LOGW(TAG, "queued alarm call to number1");
//     }
// }
...
//     if (s_alarm_last_notify_ms == 0) {
//         return;
//     }

//     if ((now_ms - s_alarm_last_notify_ms) < APP_ALARM_REMIND_MS) {
//         return;
//     }
...
//     /* 
//      * Lan 1: gui ngay khi alarm xuat hien
//      * Lan 2: sau 10 phut van con alarm -> gui SMS + CALL
//      * Tu lan 3 tro di: chi giu relay/output1 ON, khong gui nua
//      */
//     if (s_alarm_notify_count >= 3) {
//         s_alarm_last_notify_ms = now_ms;
//         ESP_LOGW(TAG, "alarm still active, keep DO1 ON only, no more SMS/call");
//         return;
//     }
...
//     s_alarm_last_notify_ms = now_ms;
//     s_alarm_notify_count++;

//     ESP_LOGW(TAG, "alarm still active after 10 minutes -> queue final reminder SMS+CALL, count=%d",
//              s_alarm_notify_count);
// }
    ...
    if (s_sht30_ready) {
        ...
        if (status == ok) {
            ...
            if (temp_is_valid(t) && !isnan(h) && !isinf(h) && h >= 0.0f && h <= 100.0f) {
                ...
                ESP_LOGW(TAG, "SHT30 data invalid temp=%.2f hum=%.2f", t, h);
            }
        } else {
            s_humidity_valid = false;
            ...
            ESP_LOGW(TAG, "SHT30 read failed, status=%d", status);
        }
    } else {
        s_humidity_valid = false;
...
case MODEM_JOB_SMS_HUM_SENSOR_RESTORED: {
    ...
    if (job.type == MODEM_JOB_SMS_TEMP_ALARM) {
    ...
    else if (job.type == MODEM_JOB_SMS_ALARM_RESTORED) {
        s_alarm_restored_sms_job_queued = false;
        if (sms_done) {
            s_alarm_restored_sms_sent = true;
            s_alarm_restored_sms_pending = false;
            ...
            s_restore_hum_pending = false;
            if (no_target) {
                ESP_LOGW(TAG, "alarm restored SMS skipped: no target");
            } else {
                ESP_LOGI(TAG, "alarm restored SMS sent to all numbers");
            }
        } else {
            ESP_LOGE(TAG, "alarm restored SMS partial/failed total=%d ok=%d fail=%d",
                     sms_res.total_enabled, sms_res.ok_count, sms_res.fail_count);
        }
    }
        ...
        if (sms_done) {
            ...
            if (no_target) {
                ...
                ESP_LOGI(TAG, "power lost SMS sent to all numbers");
            }
        } else {
        ...
        if (sms_done) {
            ...
            if (no_target) {
                ...
                ESP_LOGI(TAG, "power restored SMS sent to all numbers");
            }
        } else {
        ...
        if (sms_done) {
            ...
            if (no_target) {
                ESP_LOGW(TAG, "NTC fault SMS skipped: no target");
            } else {
                ESP_LOGI(TAG, "NTC fault SMS sent to all numbers");
            }
        } else {
            ESP_LOGE(TAG, "NTC fault SMS partial/failed total=%d ok=%d fail=%d",
                     sms_res.total_enabled, sms_res.ok_count, sms_res.fail_count);
        }
    }
        ...
        if (sms_done) {
            ...
            s_ntc_fault_sms_sent_once = false;
            if (no_target) {
                ESP_LOGW(TAG, "NTC restored SMS skipped: no target");
            } else {
                ESP_LOGI(TAG, "NTC restored SMS sent to all numbers");
            }
        } else {
            ESP_LOGE(TAG, "NTC restored SMS partial/failed total=%d ok=%d fail=%d",
                     sms_res.total_enabled, sms_res.ok_count, sms_res.fail_count);
        }
    }
        ...
        if (sms_done) {
            ...
            if (no_target) {
                ESP_LOGW(TAG, "humidity sensor fault SMS skipped: no target");
            } else {
                ESP_LOGI(TAG, "humidity sensor fault SMS sent to all numbers");
            }
        } else {
            ESP_LOGE(TAG, "humidity sensor fault SMS partial/failed total=%d ok=%d fail=%d",
                     sms_res.total_enabled, sms_res.ok_count, sms_res.fail_count);
        }
    }
            ...
            if (sms_done) {
                ...
                if (no_target) {
                    ESP_LOGW(TAG, "humidity sensor restored SMS skipped: no target");
                } else {
                    ESP_LOGI(TAG, "humidity sensor restored SMS sent to all numbers");
                }
            } else {
                ESP_LOGE(TAG, "humidity sensor restored SMS partial/failed total=%d ok=%d fail=%d",
                        sms_res.total_enabled, sms_res.ok_count, sms_res.fail_count);
            }
        }
        ...
        if (!s_new_temp_alarm_pending && !s_new_hum_alarm_pending) {
            s_alarm_sms_sent = true;
            s_alarm_sms_pending = false;
            ESP_LOGI(TAG, "all active alarm SMS jobs done");
        }
        ...
        case MODEM_JOB_POLL_INBOX: {
            ...
            } else if (sms_command_text_equals_ci(body, "2510;ENG_INFOR")) {
                sms_command_format_eng_info(reply, sizeof(reply));
                modem_service_send_sms(sender, reply);
            } else if (sms_command_text_equals_ci(body, "2510;RESET")) {
                modem_service_send_sms(sender, "RESET OK, REBOOT");
                vTaskDelay(pdMS_TO_TICKS(1500));
                handle_reboot_request_internal();
            } else if (strncmp(body, "2510;", 5) == 0) {
    ...
    while (1) {
            ...
            if (!s_power_boot_checked) {
                ...
                if (err == ESP_OK) {
                    ...
                    if (state == POWER_STATE_MAIN_LOST) {
                        ...
                        ESP_LOGW(TAG, "boot with main power lost -> queue FIRST power lost SMS/CALL");
                    }
                }
            }
        ...
        if ((now - last_publish) >= pdMS_TO_TICKS(s_pub_period_ms)) {
            if (s_uplink_ready && mqtt_service_is_connected()) {
                ...
                float hum_sht_pub  = s_last_humidity;
